...
# Design principles:
# 1. Must be idempotent (re-runnable without side-effects).
...
# The script returns `dnf` or `apt`, depending on the distribution
...
if [ -n "$PACKAGE_MANAGER" ]; then
    ...
    echo "See fd-find alias for Ubuntu in 'workstation-aliases'."
